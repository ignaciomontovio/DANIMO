...
export async function emergencyButton(userId) {
  ...
  for (const contact of emergencyContacts) {
    const msj = "Hola " + contact.name + ", somos DANIMO. " + user.firstName
        + " " + user.lastName
        + " te tiene en su lista de contactos de emergencia y está atravesando un momento difícil. " +
        "Tu apoyo puede hacer una gran diferencia, quizás podrías comunicarte con esta persona.";
...
export async function createEmergencyContact(name, phoneNumber, who, userId) {
  ...
  if (exists) {
    throw new Error(
        'Ya hay un contacto de emergencia con este número.');
  }
...
export async function updateEmergencyContact(userId, currentPhoneNumber,
    updates) {
